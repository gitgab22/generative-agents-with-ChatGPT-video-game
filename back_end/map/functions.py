...
def nearby(map, position, radius=5):
    print("recherche nearby")
    '''
...
def get_current_localisation(map, agent):
    '''
    Get the localisation of the agent
    input: map, agent
...
def prepare_map(map_file="back_end/map/map.json"):
    ...
    for building in map_dict.keys():
        for hall in map_dict[building]:
            localisations.append(building+":"+hall)
            for object in map_dict[building][hall]:
                localisations.append(building+":"+hall+":"+object)
    ...
    return width, height, case_details, map_dict, coordinates, grid_roads, localisations
